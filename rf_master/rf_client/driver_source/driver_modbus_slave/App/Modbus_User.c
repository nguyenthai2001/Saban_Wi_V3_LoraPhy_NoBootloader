...
*/
eMBErrorCode UseModbus(UCHAR slaveAddr)
{
#if USE_INIT_DEBUG > 0
    S_DBG("\r\nInit HMI Modbus");
#endif
    ...
    if ((eStatus = eMBInit(MB_RTU, slaveAddr, 0, 115200, MB_PAR_NONE)) != MB_ENOERR)    // Slave Address = 0x01
    {
#ifdef USE_DEBUG_MB
        ShowMBErrStatus(eStatus);
#endif
    ...
    //        /* ---- Set Slave ID ---- */
    if ((eStatus = eMBSetSlaveID(slaveAddr, TRUE, ucSlaveID, 3)) != MB_ENOERR)
    {
#ifdef USE_DEBUG_MB
        ShowMBErrStatus(eStatus);
#endif
