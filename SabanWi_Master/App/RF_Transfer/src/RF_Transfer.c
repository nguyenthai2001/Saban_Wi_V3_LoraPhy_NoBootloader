...
//            printf(" DECODE DATA SLAVE [%2X] OK !!! [%d] us " , SlaveID , TimeStop - TimeStart);
        ...
        if (MCCode == MCCODE_SLAVE_FEEDBACK && MasterDataFlash[1].Masterid == MasterID)                // decode for master
        {
            ...
            log_message(" SlaveID : %2X " , device[SlaveID].slaveID);
            log_message(" Command : %2X " , device[SlaveID].cmd);
            log_message(" MCcode : %2X " , device[SlaveID].mccode);
            log_message(" Data : %2X " , device[SlaveID].data_h);
            log_message("%2X " , device[SlaveID].data_l);
            err = 1 ;
        }
#endif
    }
