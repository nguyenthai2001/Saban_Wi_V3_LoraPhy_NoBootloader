...
void SB_Timer3_Init(void)
{
    ...
    TIMER_SET_CMP_VALUE(TIMER3, 0x1);           // 1us
...
{
    TIMER_Start(TIMER1);
    g_tick_timer.tick_timer1 = 0 ;
}

uint16_t Timer_1_GetTickMs(void)
{
    return g_tick_timer.tick_timer1 ;
}
...
{
    return g_tick_timer.tick_timer2 ;
}

void Timer2_ResetTickMs(void)
{
    TIMER_Stop(TIMER2);
    g_tick_timer.tick_timer2 = 0;
}
