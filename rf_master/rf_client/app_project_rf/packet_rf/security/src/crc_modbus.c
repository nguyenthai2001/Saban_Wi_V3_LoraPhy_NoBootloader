    ...
    while (size--)
    {
        ...
        for (i = 0; i < 8; i++)
        {
            if (crc & 0x01)                     // neu bit co trong so thap nhat cua CRC la 1
                crc = ((crc >> 1) ^ 0xA001);    // Dich phai 1 bit va XOR voi 0xA001
            else
                crc = (crc >> 1);               // neu bit co trong so thap nhat cua CRC la 0
        }
    }
    return crc;
}
...
void CRC_Test (void)
{
    ...
    u32Strlen = strlen(test);
    printf("Source data string is %02X , %d byte. \n\n",test , u32Strlen);
    ...
    printf("Use a CRC-32 table to generate CRC-32 result: 0x%x.\n", CRCResult);
    printf("\n");

    CRCResult = crc16_by_table(test, u32Strlen);
    printf("Use CRC-16 tables to generate CRC-16 result: 0x%x .\n", CRCResult);
    CRCResult = crc16(test, u32Strlen);
    printf("Use a software algorithm to generate CRC-16 result: 0x%x.\n", CRCResult);
    printf("\n");

    CRCResult = crc8_by_table(test, u32Strlen);
    printf("Use a CRC-8 table to generate CRC-8 result: 0x%x .\n", CRCResult );
    CRCResult = crc8(test, u32Strlen);
    printf("Use a software algorithm to generate CRC-8 result: 0x%x .\n", CRCResult );
    printf("\n");
