...
eMBErrorCode UseModbus(void)
{
    ...
    if ((eStatus = eMBInit(MB_RTU, ClientDataFlash[1].Modbus_SlaveID, 0, 115200, MB_PAR_NONE)) != MB_ENOERR)    // Slave Address = 0x01
    {
#ifdef USE_DEBUG_MB
        ShowMBErrStatus(eStatus);
#endif
    ...
    //        /* ---- Set Slave ID ---- */
    if ((eStatus = eMBSetSlaveID(ClientDataFlash[1].Modbus_SlaveID, TRUE, ucSlaveID, 3)) != MB_ENOERR)
    {
#ifdef USE_DEBUG_MB
        ShowMBErrStatus(eStatus);
#endif
